import { Box, Typography, Card, CardContent, TextField, CircularProgress } from "@mui/material";
...
const ActivityDetailsChat = observer(function ActivityDetailsChat() {
    ...
    const handleKeyPress = (event: React.KeyboardEvent<HTMLDivElement>) => {
        if (event.key === 'Enter' && !event.shiftKey) {
            ...
            handleSubmit(addComment)(); //phai co (), vi neu khong co, chi la tao ham chu khong chay
        }
    }
                    ...
                    <Box sx={{height: 400, overflow: 'auto'}}>
                        {commentStore.comments.map(comment => (
                            <Box key={comment.id} sx={{ display: 'flex', my: 2 }}>
                                <AvatarPopover  key={comment.id} id={comment.userId} />
